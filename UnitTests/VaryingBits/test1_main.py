...
from test1_library import lookup_generate, encode_HDC_RFF, evaluate_F_of_x
...
B_cnt = np.linspace(2,16,15).astype(np.uint64)
maxval = 2**B_cnt  # The input features will be mapped from 0 to 255 (8-bit)
...
N_tradeof_points = 5  # Number of tradeoff points - use 100 - original: 40
...
NM_iter = 5  # Maximum number of iterations
...
HDC_cont_all = HDC_cont_all.astype(np.int8)

# def get_optimal_point(optimalpoint):

for nbits in range(B_cnt.shape[0]):
    

    print("Progress: " + str(nbits+1) + "/" + str(B_cnt.shape[0]))
    ...
    for init_simp in range(len(Simplex)):
        ...
        local_avg, local_avgre, local_sparse = evaluate_F_of_x(Nbr_of_trials, HDC_cont_all, beta_, bias_, gamma, alpha_sp, n_class, N_train, D_b, lambda_1, lambda_2, B_cnt[nbits], Y_train, Y_test)
    ...
    for iter_ in range(NM_iter):
        ...
        local_avg_x, acc_curr, sparse_curr = evaluate_F_of_x(Nbr_of_trials, HDC_cont_all, beta_x_r, bias_, gamma_x_r, alpha_sp_x_r, n_class, N_train, D_b, lambda_1, lambda_2, B_cnt[nbits], Y_train, Y_test)
        F_r = 1 - local_avg_x
        ...
        if F_r < F_of_x[0]:  # Reflected point is best point so far
            ...
            local_avg_exp, acc_exp, sparse_exp = evaluate_F_of_x(Nbr_of_trials, HDC_cont_all, beta_exp, bias_, gamma_exp, alpha_sp_exp, n_class, N_train, D_b, lambda_1, lambda_2, B_cnt[nbits], Y_train, Y_test)
            F_e = 1 - local_avg_exp
            if F_e < F_r:
                F_of_x[-1] = F_e
                Simplex[-1] = x_e
    ...
    # F_curr >= F_of_x[-2]
        ...
        local_avg_c, acc_c, sparse_c = evaluate_F_of_x(Nbr_of_trials, HDC_cont_all, beta_c, bias_, gamma_c, alpha_sp_c, n_class, N_train, D_b, lambda_1, lambda_2, B_cnt[nbits], Y_train, Y_test)
        F_c = 1-local_avg_c
        ...
        for rep in range(1, Simplex.shape[0]):
            ...
            local_avg_s, acc_s, sparse_s = evaluate_F_of_x(Nbr_of_trials, HDC_cont_all, beta_s, bias_, gamma_s, alpha_sp_s, n_class, N_train, D_b, lambda_1, lambda_2, B_cnt[nbits], Y_train, Y_test)
            F_of_x[rep] = 1-local_avg_s
            Accs[rep] = acc_s
        # print("Elapsed time Nelder Mead:", time()-start_time)
    delta = time() - start_time
    print("Elapsed time Nelder Mead:", delta)
    print("Average per cycle:", delta/NM_iter)
    ##################################
...
"""
folder_name = "img/"+datetime.now().strftime("%m-%d_%H-%M")+str(N_tradeof_points)+"_"+str(NM_iter)
try:
    mkdir(folder_name)
...
plt.figure(1)
plt.plot(B_cnt, ACCS, 'bx', markersize=10)
plt.grid('on')
plt.xlabel("Number of bits")
plt.ylabel("Accuracy")
plt.savefig(folder_name+"/acc_bcnt.png")
