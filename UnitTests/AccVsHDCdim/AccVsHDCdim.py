...
from HDC_library_fast import lookup_generate, encode_HDC_RFF, evaluate_F_of_x
...
D_HDC = np.linspace(30,1000, 20).astype(np.uint16)  # HDC hypervector dimension
...
ACCS = np.zeros(D_HDC.shape[0])
SPARSES = np.zeros(D_HDC.shape[0])
load_simplex = True  # Keep it to true in order to have somewhat predictive results


# def get_optimal_point(optimalpoint):
for dims in range(D_HDC.shape[0]):
    print("Progress: " + str(dims+1) + "/" + str(D_HDC.shape[0]))
    ...
    grayscale_table = lookup_generate(D_HDC[dims], maxval, mode=1)  # Input encoding LUT
    position_table = lookup_generate(D_HDC[dims], imgsize_vector, mode=0)  # weight for XOR-ing
    HDC_cont_all = np.zeros((X.shape[0], D_HDC[dims]))  # Will contain all "bundled" HDC vectors

    bias_ = np.random.randint(maxval, size=D_HDC[dims])  # generate the random biases once

    for i in range(X.shape[0]):
        if i % 100 == 0:
            print(str(i) + "/" + str(X.shape[0]))
        HDC_cont_all[i, :] = encode_HDC_RFF(np.round((maxval - 1) * X[i, :]).astype(int), position_table, grayscale_table, D_HDC[dims])
    ...
    for init_simp in range(len(Simplex)):
        ...
        local_avg, local_avgre, local_sparse = evaluate_F_of_x(Nbr_of_trials, HDC_cont_all, beta_, bias_, gamma, alpha_sp, n_class, N_train, D_b, lambda_1, lambda_2, B_cnt, Y_train, Y_test)
...
"""
folder_name = "./img/"+datetime.now().strftime("%m-%d_%H-%M")+str(dims)+"_"+str(NM_iter)
try:
    mkdir(folder_name)
...
plt.figure(1)
plt.plot(D_HDC, ACCS, 'x', markersize=10)
plt.grid('on')
plt.xlabel("D_HDC")
plt.ylabel("Accuracy")
plt.savefig(folder_name+"/acc_d-hdc.png")
