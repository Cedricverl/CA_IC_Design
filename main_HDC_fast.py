...
Nbr_of_trials = 5  # Test accuracy averaged over Nbr_of_trials runs
...
lambda_sp = np.concatenate((np.linspace(-1.2, -0.2, N_fine), lambda_fine, np.linspace(0.2, 1.2, N_fine)))
...
for optimalpoint in range(N_tradeof_points):
    ...
    for iter_ in range(NM_iter):
        ...
        if F_of_x[0] <= F_r < F_of_x[-2]:
            F_of_x[-1] = np.mean(F_r)
            Simplex[-1] = x_r
            Accs[-1] = np.mean(acc_curr)
            Sparsities[-1] = np.mean(sparse_curr)
            continue
        ...
        if F_r < F_of_x[0]:  # Reflected point is best point so far
            ...
            if F_e < F_r:
                F_of_x[-1] = np.mean(F_e)
                Simplex[-1] = x_e
                Accs[-1] = np.mean(acc_exp)
                Sparsities[-1] = np.mean(sparse_exp)
                continue
            else:
                F_of_x[-1] = np.mean(F_r)
                Simplex[-1] = x_r
      ...
      # F_curr >= F_of_x[-2]
        ...
        if (F_c < F_r and flag == 0) or (F_c < F_of_x[-1] and flag == 1):
            F_of_x[-1] = np.mean(F_c)
            Simplex[-1] = x_c
            Accs[-1] = np.mean(acc_c)
            Sparsities[-1] = np.mean(sparse_c)
            continue
        ...
        for rep in range(1, Simplex.shape[0]):
            ...
            F_of_x[rep] = np.mean(1-local_avg_s)
            Accs[rep] = np.mean(acc_s)
            Sparsities[rep] = np.mean(sparse_s)
...
plt.savefig(folder_name+"/acc_spar.png")
...
plt.grid("on")
plt.savefig(folder_name+"/obj_std.png")

plt.figure(3)
plt.plot(lambda_sp, ACCS)
plt.xlabel("accuracy")
plt.savefig(folder_name+"/acc.png")

plt.figure(4)
plt.plot(lambda_sp, SPARSES)
plt.xlabel("sparsity)")
plt.savefig(folder_name+"/sparsity.png")
