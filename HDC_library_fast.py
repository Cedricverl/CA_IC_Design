...
def evaluate_F_of_x(Nbr_of_trials, HDC_cont_all, beta_, bias_, gamma, alpha_sp, n_class, N_train, D_b, lambda_1, lambda_2, B_cnt, Y_train, Y_test):
    ...
    for trial_ in range(Nbr_of_trials):
        ...
        HDC_cont_train_cpy = (HDC_cont_train_cpy*beta_).astype(np.int16)    # 16 is needed because there could a bias = to 256 or near there that summed would give a wrong value
        ...
        HDC_cont_test_cpy = (HDC_cont_test_cpy*beta_).astype(np.int16)  # 16 is needed because there could a bias = to 256 or near there that summed would give a wrong value
